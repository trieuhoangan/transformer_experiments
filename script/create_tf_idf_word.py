...
def count_appearance(words,dictionary):
    ...
    for i in range(num_word):
        word = words[i]
        word,time = check_word_in_dict(word,dictionary)
        dictionary[word] = time+1
        return dictionary
def extract_tf_from_input_file(input_file,dictionary,contain_doc_dictionary):
...
if __name__=="__main__":
    ...
    if args.train_file is not None:
        dictionary,contain_doc_dictionary,num_doc = extract_tf_from_input_file(args.train_file,dictionary,contain_doc_dictionary)
        total_doc = total_doc +num_doc
    if args.test_file is not None:
        dictionary,contain_doc_dictionary,num_doc = extract_tf_from_input_file(args.test_file,dictionary,contain_doc_dictionary)
        total_doc = total_doc +num_doc
    if args.valid_file is not None:
        dictionary,contain_doc_dictionary,num_doc = extract_tf_from_input_file(args.valid_file,dictionary,contain_doc_dictionary)
        total_doc = total_doc +num_doc
