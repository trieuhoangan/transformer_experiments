...
class LISACrossEntropyCriterion(LegacyFairseqCriterion):
    ...
    def __init__(self, args, task):
        ...
        self.sub_penalty = args.sub_penalty
        self.lisa_layer = args.encoder_lisa_layer
        self.encoder_layers = args.encoder_layers
        self.src_tags_dict = task.source_tags_dictionary
        self.sub_tags_dict = task.sub_tags_dictionary
        # self.sub_dim = (len(self.sub_tags_dict)-4)*2
        self.sub_dim = 60
    ...
    def add_args(parser):
        ...
        parser.add_argument('--sub-penalty', default=0.3, type=float,
                            help='penalty of sub loss')
        # fmt: on
    ...
    def forward(self, model, sample, reduce=True):
        ...
        sub_tags = sample['net_input']['sub_tags']
        l = sub_tags.size(1) - 1
        subs = torch.cuda.LongTensor(np.vectorize(lambda e: self.map_dictionary.get(e, l))(sub_tags.cpu()))
        rng = torch.cuda.FloatTensor(range(self.sub_dim))
        sub_target = torch.lt(torch.abs(rng - subs.unsqueeze(-1)), 1).type(torch.cuda.FloatTensor)
        # sub_target = subs
        sub_prob = net_output[3]
        # print("sub_dim from crossentropy ",self.sub_dim)
        # print("from model ",sub_prob.shape)
        # print("from crossentropy ",sub_target.shape)
        sub_loss = F.mse_loss(sub_prob, sub_target, reduction='sum')
        # Combine losses
        multi_loss = loss + self.parse_penalty * attn_loss + self.sub_penalty * sub_loss
        ...
        logging_output = {
            ...
            'sub_loss': utils.item(attn_loss.data) if reduce else sub_loss.data,
