...
def load_tag_langpair_dataset(
    ...
    for dk, data_paths in enumerate(tags_data):
        for k in itertools.count():
            ...
            if not combine:
                break
    for dk, data_paths in enumerate(sub_tags):
        for k in itertools.count():
            split_k = split + (str(k) if k > 0 else '')
            # infer langcode
            if split_exists(split_k, src, tgt, src, data_paths):
            ...
            sub_tags_datasets.append(data_utils.load_indexed_dataset(prefix + src, sub_tags_dict,dataset_impl))
    ...
    if len(src_datasets) == 1:
        ...
        sub_tags_dataset = sub_tags_datasets[0]
    else:
        sample_ratios = [1] * len(src_datasets)
        sample_ratios[0] = upsample_primary
        src_dataset = ConcatDataset(src_datasets, sample_ratios)
        src_tags_dataset = ConcatDataset(src_tags_datasets, sample_ratios)
        sub_tags_dataset = ConcatDataset(sub_tags_datasets, sample_ratios)
    ...
    if prepend_bos:
        ...
        sub_tags_dataset = PrependTokenDataset(sub_tags_dataset, sub_tags_dict.bos())
        if tgt_dataset is not None:
    ...
    if append_source_id:
        ...
        sub_tags_dataset = AppendTokenDataset(
            sub_tags_dataset, sub_tags_dict.index("[{}]".format(src))
        )
        if tgt_dataset is not None:
...
class TagsTranslationTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--sub-tags', nargs='+', default=None, 
                            help='path(s) to tags data directorie(s)')
    ...
    @staticmethod
    def load_pretrained_model(path, src_dict_path, tgt_dict_path, src_tags_dict_path,sub_tag_dict_path, arg_overrides=None):
        ...
        sub_tags_dict = Dictionary.load(sub_tag_dict_path)
        task = TagsTranslationTask(args, src_dict, tgt_dict, src_tags_dict,sub_tags_dict)
        model = task.build_model(args)
        model.upgrade_state_dict(state_dict)
        model.load_state_dict(state_dict, strict=True)
        return model

    def __init__(self, args, src_dict, tgt_dict, src_tags_dict,sub_tags_dict):
        super().__init__(args)
    ...
    def setup_task(cls, args, **kwargs):
        ...
        sub_tags_dict = Dictionary.load(os.path.join(args.sub_tags[0], 'dict.{}.txt'.format(args.source_lang)))
        print('| [{}] tags dictionary: {} types'.format(args.source_lang, len(src_tags_dict)))

        return cls(args, src_dict, tgt_dict, src_tags_dict,sub_tags_dict)
    ...
    @property
    def sub_tags_dictionary(self):
        """Return the source tags :class:`~fairseq.data.Dictionary`."""
        return self.sub_tags_dict
