...
class PascalTransformerEncoder(FairseqTagsEncoder):
    ...
    def forward(self, src_tokens, src_lengths, src_tags):
        ...
        parents = torch.cuda.FloatTensor(np.vectorize(lambda e: self.map_dictionary.get(e, maxlen))(src_tags.cpu()))
        ...
        for i, h in enumerate(self.encoder_pascal_heads):
            if h != 0:
                ...
                x = self.layers[i](x, encoder_padding_mask, parents)
                    # print("alolalalala")
                # except:
                #     # print("this is x in debug message ",x.shape)
                #     # print("this is shape of parents in debug message ",parents.shape)
                #     # print("this is src_tags size ",src_tags.shape)
                #     # print("this is src_tags ",src_tags)
                #     # print("this is parents in debug message ",parents)
                #     # with open("note.txt",'a') as flog:
                #     #     flog.write("1")
                #     print("1")
            else:
...
class PascalTransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask, parents):
        ...
        # except:
        #     print("this is x in debug message ",x.shape)
        #     print("this is parents in debug message ",parents.shape)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
